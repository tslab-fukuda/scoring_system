...
const TEACHER_DASHBOARD_STATE_KEY = 'teacherDashboardState';
...
new Vue({
    ...
    methods: Object.assign({}, offeringSelectorHelper.methods, {
        saveDashboardState() {
            ...
            sessionStorage.setItem(TEACHER_DASHBOARD_STATE_KEY, JSON.stringify(payload));
        },
        restoreDashboardState() {
            const raw = sessionStorage.getItem(TEACHER_DASHBOARD_STATE_KEY);
            if (!raw) return;
            try {
                ...
                console.warn('failed to restore teacher dashboard state', e);
            } finally {
